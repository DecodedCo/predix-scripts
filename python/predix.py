...
def getAnalyticsRuntimeURLandZone(config):
	if not hasattr(config,'RUNTIME_URI') :
		runtimeUri = ''
		d = json.loads(config.formattedJson)
		runtimeUri = d[0]['VCAP_SERVICES'][config.predixAnalyticsRuntime][0]['credentials']['execution_uri']
		config.RUNTIME_URI = runtimeUri

def getAnalyticsCatalogURLandZone(config):
	if not hasattr(config,'CATALOG_URI') :
		catalogUri = ''
		d = json.loads(config.formattedJson)
		catalogUri = d[0]['VCAP_SERVICES'][config.predixAnalyticsCatalog][0]['credentials']['catalog_uri']
		config.CATALOG_URI = catalogUri
