...
then
	...
	echo "Machine Templates Github URL : $MACHINE_TEMPLATES_GITHUB_REPO_URL"
	#echo "Removing $machineTemplatesRepoName"
	if [ ! -d $machineTemplatesRepoName ]
	then
		git clone $MACHINE_TEMPLATES_GITHUB_REPO_URL --recursive
	fi
	#Unzip the original PredixMachine container
	rm -rf $PREDIX_MACHINE_HOME
	unzip -o $machineTemplatesRepoName/PredixMachine.zip -d $PREDIX_MACHINE_HOME
	...
	cp $SCRIPT_RUN_DIR/target/predix-machine-template-adapter-edison-1.0.jar $PREDIX_MACHINE_HOME/machine/bundles
	cp $SCRIPT_RUN_DIR/config/solution.ini $PREDIX_MACHINE_HOME/machine/bin/vms
	cp $SCRIPT_RUN_DIR/config/pm_background.sh $PREDIX_MACHINE_HOME/machine/bin/predix
	cp $SCRIPT_RUN_DIR/config/com.ge.dspmicro.hoover.spillway-0.config $PREDIX_MACHINE_HOME/configuration/machine
	cp $SCRIPT_RUN_DIR/config/com.ge.predix.solsvc.workshop.adapter.config $PREDIX_MACHINE_HOME/configuration/machine
	cp $SCRIPT_RUN_DIR/config/com.ge.predix.workshop.nodeconfig.json $PREDIX_MACHINE_HOME/configuration/machine
	chmod 775 $PREDIX_MACHINE_HOME/machine/bin/predix/*
	echo "Removing $machineTemplatesRepoName"
